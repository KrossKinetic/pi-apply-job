...
export function writeApprovalPage(folder: string): string {
  ...
  const job = readJsonFile<{ company: string; role: string }>(path.join(folder, "metadata.json"));
  ...
  const blocked = state.humanReviewRequired ? `<p><strong>Human review required:</strong> ${escapeHtml(state.humanReviewRequired)}</p>` : "";
  const html = `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src 'self' data:; style-src 'unsafe-inline'; object-src 'self'; base-uri 'none'; form-action 'none'"><title>Review résumé</title><style>body{font:16px/1.5 system-ui;margin:24px;color:#20252c;background:#f7f8fa}main{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:24px}article{min-width:0}img{width:100%;height:auto}pre{white-space:pre-wrap;overflow-wrap:anywhere;background:white;padding:16px}li{margin:8px 0}a{color:#075ba8}@media(max-width:850px){main{display:block}}</style><h1>${escapeHtml(job.company)} — ${escapeHtml(job.role)}</h1><p>Review the PDF and supporting factual evidence. Closing this page does not approve the résumé.</p>${blocked}<p><a href="resume.pdf">Open PDF</a> · <a href="claim-ledger.json">Claim ledger</a></p><main><article><img src="resume-preview.png" alt="Rendered résumé preview"></article><article><h2>Selected entries</h2><ul>${entries}</ul><h2>Quote-validated job brief</h2><pre>${show("job-requirement.json")}</pre><h2>Independent factual findings</h2><pre>${show("independent-verification.json")}</pre><h2>Layout checks</h2><pre>${show("layout.json")}</pre><details><summary>Claim sources</summary><pre>${show("claim-ledger.json")}</pre></details><details><summary>Optional cover letter</summary><pre>${show("cover-letter.md")}</pre></details></article></main></html>`;
