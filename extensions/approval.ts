...
import { planEntries } from "./worker-submissions.js";
...
export function writeApprovalPage(folder: string): string {
  ...
  const entries = planEntries(plan).map(e => `<li><strong>${escapeHtml(e.title)}</strong> — ${escapeHtml("subtitle" in e ? e.subtitle || "" : "Project")}<ul>${e.bullets.map(b=>`<li>${escapeHtml(b.text)}</li>`).join("")}</ul></li>`).join("");
  const qualityLimit = state.qualityLimitReached ? `<p><strong>Quality-review cap reached:</strong> this factually verified candidate is ready for your judgment. The unresolved reviewer suggestion is shown below.</p>` : "";
  const html = `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src 'self' data:; style-src 'unsafe-inline'; object-src 'self'; base-uri 'none'; form-action 'none'"><title>Review résumé</title><style>body{font:16px/1.5 system-ui;margin:24px;color:#20252c;background:#f7f8fa}main{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:24px}article{min-width:0}img{width:100%;height:auto}pre{white-space:pre-wrap;overflow-wrap:anywhere;background:white;padding:16px}li{margin:8px 0}a{color:#075ba8}@media(max-width:850px){main{display:block}}</style><h1>${escapeHtml(plan.target.company)} — ${escapeHtml(plan.target.role)}</h1><p>Review the PDF and supporting evidence. Return to Pi to approve, request a revision, or lock selected entries. Closing this page does not approve the résumé.</p>${qualityLimit}<p><a href="resume.pdf">Open PDF</a> · <a href="claim-ledger.json">Claim ledger</a></p><main><article><img src="resume-preview.png" alt="Rendered résumé preview"></article><article><h2>Selected entries</h2><ul>${entries}</ul><h2>Requirement coverage and excluded alternatives</h2><pre>${show("quality-review.json")}</pre><h2>Independent factual findings</h2><pre>${show("independent-verification.json")}</pre><h2>Layout checks</h2><pre>${show("layout.json")}</pre><details><summary>Claim sources</summary><pre>${show("claim-ledger.json")}</pre></details><details><summary>Optional cover letter</summary><pre>${show("cover-letter.md")}</pre></details></article></main></html>`;
